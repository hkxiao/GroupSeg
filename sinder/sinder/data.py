...
class GroupDataset(Dataset):
    def __init__(self, transform, dataset):
        self.transform = transform
        self.files = glob(os.path.join('data',dataset,'img','*','*.jpg'))
        # self.files = glob(os.path.join('data',dataset,'img','binder','*.jpg'))
        # print(self.files)
...
def load_group_data(args, model):
    transform = make_transform(
        args.visual_size, model.patch_size, max_edge_size=1792
    )
    dataset = GroupDataset(transform=transform, dataset=vars(args).get('dataset'))
    return dataset