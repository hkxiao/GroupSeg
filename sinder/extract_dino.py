...
def parse_args():
    ...
    parser.add_argument(
        '--dataset', type=str, default='CoCA', help='path to image/images'
    )
    parser.add_argument(
...
def visualize(args, model, visual_dataset):
    ...
    for d in tqdm(range(len(visual_dataset))):
        visual_image, image_path = visual_dataset[d]
        ...
        save_path = image_path.replace('img', 'sinder').replace('jpg', 'pth')
        Path(os.path.dirname(save_path)).mkdir(parents=True, exist_ok=True)
        torch.save(visual_tokens[0], save_path)
        print(save_path)
        continue
...
def main():
    ...
    visual_dataset = load_group_data(args, model)
