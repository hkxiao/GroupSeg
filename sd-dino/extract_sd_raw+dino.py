...
def compute_pck(model, aug, extractor, layer, facet, files):
    ...
    for idx in tqdm(range(N)):
        if not files[idx].endswith('.jpg') and  not files[idx].endswith('.png'): continue
        ...
        with torch.no_grad():
            ...
            torch.save(state_dict,files[idx].replace('img','sd_raw+dino_feat')[:-4]+'.pth')
...
def main(args):
    ...
    # prepare path
    imgs_dir = os.path.join(args.path, 'img')
